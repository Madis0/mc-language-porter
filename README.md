...
[Otsid eesti keelte pakki? Vaata siit.](/Juhised.md)
...
* `(language).lang` - A Bedrock Edition language file, not included in this repo. English one downloadable by `downloader.py` or can be obtained [from official resources](https://github.com/Mojang/bedrock-samples). Translated one generated by `porter.py`.
* `extra-et_EE.lang` - The extra mappings for Estonian languages pack. You can also create one for other languages by using a similar format.
...
* `Juhised.md` - Download instructions for the Estonian languages pack.
* Folder `Test` - A set of tests that can be used with `mapper.py` to verify that it is working correctly.
* Folder `Eesti keelte pakk` - Structure of the Estonian languages pack, actual language file must be generated with the scripts.
